...
const ipTracker = function (ipAddress) {
  fetch(
    `https://geo.ipify.org/api/v2/country,city?apiKey=at_RBu2rsepucyUi00nDvJatyeJsIDt3&ipAddress=${ipAddress}`
  )
    ...
    .catch((err) => {
      document.getElementById(
        "responseDiv"
      ).innerHTML = `<p style=color:red>Please Enter a valid ip address</p>`;
...
const getMap = (data, ipAddress) => {
  ...
  let marker = new L.marker([data.location.lat, data.location.lng])
    .addTo(map)
    .bindPopup("Your ip address.<br> Easily customizable.")
    .openPopup();
  map.addLayer(layer);
};
//call getip method
